...
# Create table to save user data (if it doesn't exist)
c.execute("""
...
if st.button("Predict Loan Status"):
    if customer_id.strip() == "":
        ...
        INSERT INTO loan_applications (
            customer_id, age, income, home_ownership, loan_amount, 
            loan_interest_rate, credit_score, previous_defaults, loan_status, application_date
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (customer_id, person_age, person_income, person_home_ownership, loan_amnt, 
              loan_int_rate, credit_score, previous_loan_defaults_on_file, loan_status, application_date))

        # Commit the transaction
        conn.commit()

        # Show success message
        st.info("Your loan application has been submitted and saved to the database.")
